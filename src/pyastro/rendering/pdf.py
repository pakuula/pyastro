"""Генерация PDF отчёта по гороскопу"""
import shutil
import subprocess
import tempfile
...
def export_as_pdf(
    ...
    pandoc_defaults_file: Optional[str] = None,
) -> None:
    ...
    if pandoc_defaults_file is None:
        if not _which("fc-list"):
            raise ValueError(
                "Команда 'fc-list' не найдена в PATH, требуется для проверки шрифтов"
            )

        for font in ["FreeSerif", "FreeSans", "FreeMono"]:
            if not _has_font(font):
                raise ValueError(
                    f"Шрифт '{font}' не найден в системе, установите его: https://www.gnu.org/software/freefont/"
                )
...
def to_pdf_weasy(chart: Chart, svg_chart: str, pdf_path: str) -> None:
    ...
    try:
        ...
        raise ValueError(
            "WeasyPrint не установлен, установите его: pip install weasyprint"
        ) from e
